...
pub struct DesmosQuerier<'a> {
    querier: &'a QuerierWrapper<'a, DesmosQueryWrapper<'a>>,
}

impl <'a> DesmosQuerier<'a> {
    pub fn new(querier: &'static QuerierWrapper<'static, DesmosQueryWrapper>) -> Self {
        DesmosQuerier { querier }
    }

    pub fn query_post(&self) -> StdResult<PostsResponse> {
    ...
    pub fn query_post_reports(&self) -> StdResult<ReportsResponse> {
        let request = DesmosQueryWrapper {
            route: DesmosRoute::Posts,
